...
public class UserProvider {
    ...
    public String findUsers(UserDto uInfo) {
        ...
        sql.LEFT_OUTER_JOIN("(SELECT u_id,r_id FROM `wh_user_role_user` WHERE is_delete=0) AS ur ON ur.u_id=u.uid");
        sql.LEFT_OUTER_JOIN("(SELECT r_id,r_name FROM `wh_user_role` WHERE is_delete=0) AS r ON r.r_id=ur.r_id");
        //用户类型
