...
public class WhUserPermsProvider {
    ...
    public String roleQueryPermission(Map<String, String> strMap) {
        String rids = strMap.get("rids");
        SQL sql = new SQL();
        sql.SELECT("p.`p_id`,p.`p_name` FROM `wh_user_perms` AS p");
        sql.LEFT_OUTER_JOIN("(SELECT r_id,p_id FROM `wh_user_role_perms` WHERE is_delete=0) AS rp ON rp.`p_id` =p.`p_id`");
        return sql.toString() + " WHERE " + StrUtils.in(rids, "rp.`r_id`") +
                "AND " + ALIAS + ".is_delete=0 GROUP BY  p.p_id";
    }
    ...
    public String getPermissionAndOperating(Map<String, Object> strMap) {
        ...
        return sql.toString() + " WHERE " + StrUtils.in(rids, "rp.`r_id`") +
                "AND " + ALIAS + ".is_delete=0  GROUP BY " + ALIAS + ".`p_id` ";
    }
}
