...
public class ExcelTool<T> {
    ...
    private void init(int styleFlag) {
        ...
        this.styleHead.setRightBorderColor(IndexedColors.AQUA.getIndex());
        this.styleHead.setBottomBorderColor(IndexedColors.AQUA.getIndex());
        switch (styleFlag) {
            case 1:
                ...
                this.styleBody.setRightBorderColor(IndexedColors.AQUA.getIndex());
                this.styleBody.setBottomBorderColor(IndexedColors.AQUA.getIndex());
                this.styleBody.setBorderRight(BorderStyle.MEDIUM_DASHED);// 边框的大小
                this.styleBody.setBorderBottom(BorderStyle.MEDIUM_DASHED);// 边框的大小
                break;
            default:
                this.styleBody = this.workbook.createCellStyle();
                this.styleBody.setAlignment(HorizontalAlignment.CENTER);// 左右居中ALIGN_CENTER
...
//                this.styleBody.setBorderBottom(BorderStyle.MEDIUM_DASHED);// 边框的大小
     ...
     */
    private void writeSheetContent(List<Column> listTpamscolumn, List<T> datas, XSSFSheet sheet, int rowIndex, boolean rowFlag) throws Exception {
        XSSFRow row = null;
...
//            RegionUtil.setBorderBottom(BorderStyle.MEDIUM_DASHED, cra, sheetCo); // 下边框
//            RegionUtil.setBorderLeft(BorderStyle.MEDIUM_DASHED, cra, sheetCo); // 左边框
//            RegionUtil.setBorderRight(BorderStyle.MEDIUM_DASHED, cra, sheetCo); // 有边框
     ...
     * @param fileName 填写行内数据对的 字段名称
     * @param rootId   rootid的值
     * @return
     * @throws Exception
     */
    public List<Column> columnTransformer(List<T> list, String id, String pid, String content, String fileName, String rootId) throws Exception {
        List<Column> lc = new ArrayList<>();
        if (list.get(0) instanceof Map) {
            for (Map m : (List<Map>) list) {
                ...
                m.forEach((k, val) -> {//java8 以上的遍历方式
                    ...
                    if (pid.equals(k)) tpamscolumn.setPid(String.valueOf(val));
                    if (content.equals(k)) tpamscolumn.setContent((String) val);
                    if (fileName.equals(k) && fileName != null) tpamscolumn.setFieldName((String) val);
                });
                lc.add(tpamscolumn);
            }
        } else {
...
//                    if (pid.equals(f.getName()) && ( f.get(t) == null || "".equals(f.get(t)))) tpamscolumn.setPid("0");
                    ...
                    if (f.get(t) != null && fileName != null && fileName.equals(f.getName()))
...
//========上部分是导出excel的使用（生成excel），下部分是解析excel，由于excel导入==================================================================================================================================
    ...
    public List<List<String>> getExcelValues(String fileUrl, int sheetNum) throws Exception {
        ...
        Workbook workbook = WorkbookFactory.create(new FileInputStream(new File(fileUrl)));
    ...
    private static boolean CheckRowNull(Row row, int maxRow) {
        ...
        for (int j = 0; j < maxRow; j++) {
            Cell cell = row.getCell(j);
            if (cell == null || cell.equals("") || cell.getCellType() == BLANK) {
                num++;
            }
        }
    ...
    public List<List<Map<String, String>>> getExcelMapVal(String fileUrl, int sheetNum) throws Exception {
        List<List<Map<String, String>>> values = new ArrayList<>();
        Workbook workbook = WorkbookFactory.create(new FileInputStream(new File(fileUrl)));
        int sheetCount = sheetNum - 1; //workbook.getNumberOfSheets();//sheet 数量,可以只读取手动指定的sheet页
        //int sheetCount1= workbook.getNumberOfSheets();
        Sheet sheet = workbook.getSheetAt(sheetCount); //读取第几个工作表sheet
        ...
        for (int i = 1; i <= rowNum; i++) {
            ...
            for (int j = 0; j < colCount; j++) {//第j列://+1是因为最后一列是空 也算进去
                ...
                String cellValue;
                String cellKey = getCellValue(cellTitle);
                boolean isMerge = false;
                if (cell != null) {
                    isMerge = isMergedRegion(sheet, i, cell.getColumnIndex());
                }
